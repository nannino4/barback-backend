...
describe('Integration Tests', () => 
{
    ...
    describe('User-Organization Integration', () => 
    {
        it('should create user and add them to organization', async () => 
        {
            ...
            expect((userOrgs[0].userId as any)._id.toString()).toBe((user._id as Types.ObjectId).toString());
        ...
        it('should handle user removal from organization', async () => 
        {
            ...
            expect((orgUsers[0].userId as any)._id.toString()).toBe((user1._id as Types.ObjectId).toString());
        });
    ...
    describe('User-Subscription Integration', () => 
    {
        ...
        it('should handle subscription tier changes', async () => 
        {
            ...
            // Activate the subscription first (since new subscriptions are PENDING by default)
            const activatedSubscription = await subscriptionService.reactivate((subscription._id as Types.ObjectId).toString());
            ...
            const updatedSubscription = await subscriptionService.changeTier(
                (activatedSubscription._id as Types.ObjectId).toString(),
                changeTierDto
            );
    ...
    describe('Organization-Subscription Integration', () => 
    {
        ...
        it('should handle organization cleanup when removing users', async () => 
        {
            ...
            expect(orgUsers).toHaveLength(1);
            expect((orgUsers[0].userId as any)._id.toString()).toBe((owner._id as Types.ObjectId).toString());
        });
    });
    ...
    describe('Complex Data Operations', () => 
    {
        it('should handle user with multiple organizations and subscription', async () => 
        {
            ...
            expect(org1Users.some(u => (u.userId as any)._id.toString() === (user._id as Types.ObjectId).toString())).toBe(true);
            expect(org2Users.some(u => (u.userId as any)._id.toString() === (user._id as Types.ObjectId).toString())).toBe(true);
            expect((userSubscription.userId as any)._id.toString()).toBe((user._id as Types.ObjectId).toString());
        });
    });
