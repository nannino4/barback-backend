...
```json
{
  ...
  "state": "state_parameter_for_validation"  // Required
}
```

**Validation Rules**:
- `code`: Required, non-empty string
- `state`: Required, non-empty string (signed JWT generated by backend)
...
**401 Unauthorized** - Invalid OAuth State:
```json
{
...
**Note**: State validation protects against CSRF attacks. State must be a valid signed JWT generated by the backend.
...
- Profile pictures are set from Google on first signup/account linking only
- Users can manually update their profile picture after initial setup
